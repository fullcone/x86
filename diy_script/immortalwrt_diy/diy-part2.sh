...
# fullcone smartdns feed override
install_smartdns_feed_package() {
    local url="$1"
    local target_dir="$2"
    ...
    case "$target_dir" in
        feeds/packages/net/smartdns|feeds/luci/applications/luci-app-smartdns) ;;
        *)
            echo "Refusing unexpected SmartDNS target directory: $target_dir"
            exit 1
            ;;
    esac

    tmp_dir="$(mktemp -d)"
    wget "$url" -O "$tmp_dir/master.zip"
    unzip -q "$tmp_dir/master.zip" -d "$tmp_dir"
    test -d "$tmp_dir/$extracted_dir" || {
        echo "Missing extracted SmartDNS package directory: $tmp_dir/$extracted_dir"
        exit 1
    }
...
install_smartdns_feed_package \
    "https://github.com/pymumu/openwrt-smartdns/archive/master.zip" \
    "feeds/packages/net/smartdns" \
    "openwrt-smartdns-master"

install_smartdns_feed_package \
    "https://github.com/pymumu/luci-app-smartdns/archive/master.zip" \
    "feeds/luci/applications/luci-app-smartdns" \
    "luci-app-smartdns-master"
...
if [ -f feeds/packages/net/smartdns/Makefile ]; then
    ...
    echo "SmartDNS PKG_MIRROR_HASH forced to skip"
    grep -nE '^(PKG_VERSION|PKG_SOURCE_VERSION|PKG_MIRROR_HASH):=' feeds/packages/net/smartdns/Makefile
else
    echo "Missing SmartDNS Makefile after feed override"
    exit 1
fi
